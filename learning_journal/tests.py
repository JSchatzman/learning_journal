...
def set_auth_credentials():
    """Make username/password for testing."""
    import os
...
@pytest.fixture(scope='function')
def login_testcase(testapp):
    """Pass login credentials for test cases."""
    import os
    from passlib.apps import custom_app_context as pwd_context
    os.environ['AUTH_USERNAME'] = 'testname'
    os.environ['AUTH_PASSWORD'] = pwd_context.hash('testpassword')
...
def test_create_new_page_exists(testapp, login_testcase):
...
def test_for_home_link_in_update(testapp, login_testcase):
    """Test home page link exists in detail page."""
...
def test_for_home_link_in_new(testapp, login_testcase):
...
def test_access_denied_on_edit_if_no_auth(testapp):
    """Test that access is denied if not authenticated."""
    response = testapp.get("/journal/2/edit-entry", status=403)


def test_access_denied_on_create_if_no_auth(testapp):
    """Test that access is denied if not authenticated."""
    response = testapp.get("/journal/new-entry", status=403)


def test_access_denied_once_signed_out(testapp, login_testcase):
    """Test that access is denied once signed out."""
    testapp.get('/logout')
